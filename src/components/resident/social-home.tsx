...
type FriendCard = Author & {
  status?: "FRIEND" | "INCOMING" | "SENT" | "SUGGESTION";
  mutualCount?: number;
  requestId?: string;
};
...
export function SocialHome({ initialView = "home" }: { initialView?: string }) {
  ...
  function readMedia(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    files.forEach((file) => {
      ...
      reader.onload = () =>
        setMedia((current) => [...current, String(reader.result)].slice(0, 4));
  ...
  async function publish() {
    ...
    const response = await fetch("/api/social/feed", {
      ...
      body: JSON.stringify({
        body: composer,
        mediaUrls: media,
        isVideo: media.some((item) => item.startsWith("data:video")),
      }),
    });
  ...
  async function sendComment(postId: string) {
    const body = comment[postId]?.trim();
    if (!body) return;
    const response = await fetch("/api/social/interactions", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ postId, comment: body }),
    });
    if (response.ok) {
  ...
  const filteredPosts =
    view === "videos"
      ? posts.filter(
          (post) =>
            post.isVideo ||
            mediaList(post.mediaUrls).some((item) => item.includes("video")),
        )
      : posts;
          ...
          <main className="space-y-5">
            <div>
              <p className="text-sm font-semibold uppercase tracking-[0.18em] text-primary">
                {view === "videos" ? "Watch together" : "Your community"}
              </p>
              <h1 className="mt-1 text-3xl font-bold">
                {view === "videos" ? "Video feed" : "Good morning, resident"}
              </h1>
              <p className="mt-1 text-muted-foreground">
                {view === "videos"
                  ? "Short videos shared by people in your community."
                  : "A closer, kinder feed for the people around you."}
              </p>
            </div>
            {view === "home" && (
              <div className="flex gap-3 overflow-x-auto pb-1">
                <button
                  className="grid h-28 min-w-20 place-items-center rounded-2xl border-2 border-dashed border-primary/30 bg-primary/5 p-2 text-center text-xs font-semibold text-primary"
                  onClick={() =>
                    toast.info("Create a story from the composer below")
                  }
                >
                ...
                {stories.map((story) => (
                  <div
                    key={story.id}
                    className="relative grid h-28 min-w-20 place-items-end overflow-hidden rounded-2xl bg-gradient-to-br from-primary to-secondary p-2 text-left text-xs font-semibold text-white"
                  >
                    <Avatar className="absolute left-2 top-2 h-8 w-8 ring-2 ring-white">
                ...
                <CardContent className="p-4">
                  <div className="flex gap-3">
                    ...
                    <Textarea
                      ...
                      className="min-h-20 resize-none border-0 bg-muted/50 shadow-none"
                    />
                  </div>
                  <div className="mt-3 flex items-center justify-between gap-3 border-t pt-3">
                    ...
                    <Button
                      size="sm"
                      onClick={publish}
                      disabled={!composer.trim() && media.length === 0}
                    >
                  ...
                  {media.length > 0 && (
                    <p className="mt-2 text-xs text-muted-foreground">
                      {media.length} media item(s) ready to share
                    </p>
                  )}
                </CardContent>
              </Card>
                      ...
                      <Button
                        ...
                        onClick={() =>
                          navigator.clipboard?.writeText(
                            `${window.location.origin}/resident/home#${post.id}`,
                          )
                        }
                      >
                        <Send className="mr-2 h-4 w-4" />
                    ...
                    <div className="space-y-2 bg-muted/30 px-4 py-3">
                      {post.comments.map((item) => (
                        <p key={item.id} className="text-sm">
                          <b>{item.author.name ?? "Resident"}</b> {item.body}
                        </p>
                      ))}
            ...
            <Card className="bg-gradient-to-br from-primary/10 to-secondary/10">
              <CardContent className="p-5">
                ...
                <p className="mt-2 text-sm leading-6 text-muted-foreground">
                  Share recycling wins, find neighbors, and keep your barangay
                  moving forward.
                </p>
              </CardContent>
            </Card>
...
function SectionPanel({ view }: { view: string }) {
      ...
      .then((data) =>
        setItems(
          view === "friends"
            ? [
                ...(data.friends ?? []),
                ...(data.requests ?? []).map((item: FriendCard) => ({
                  ...item,
  ...
  async function createListing() {
    ...
    const response = await fetch("/api/social/marketplace", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        title: listingTitle,
  ...
  function handleListingImage(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 6 * 1024 * 1024)
      return toast.error("The item image must be smaller than 6 MB.");
    const reader = new FileReader();
  ...
  async function sendMessage() {
    ...
    const response = await fetch("/api/social/messages", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ conversationId: items[0].id, body: message }),
    });
  ...
  async function updateFriendRequest(
    ...
    if (response.ok) {
      const refreshed = await fetch("/api/social/friends").then((res) =>
        res.json(),
      );
      setItems([
        ...(refreshed.friends ?? []),
  ...
  async function sendFriendRequest(receiverId: string) {
    ...
    if (response.ok) {
      ...
      setItems((current) =>
        current.map((item) =>
          item.id === receiverId
            ? { ...item, status: "SENT", requestId: request.id }
            : item,
        ),
      );
  ...
  async function removeFriendRequest(requestId: string, receiverId: string) {
    const response = await fetch(
      `/api/social/friends?requestId=${encodeURIComponent(requestId)}`,
      { method: "DELETE" },
    );
    if (response.ok) {
      setItems((current) =>
        current.map((item) =>
          item.id === receiverId
            ? { ...item, status: "SUGGESTION", requestId: undefined }
            : item,
        ),
      );
    ...
    return (
      <div className="rounded-[2rem] border border-border/60 bg-card p-8 text-center shadow-sm">
        ...
        <p className="mt-2 text-muted-foreground">
          Manage your personal information and community identity.
        </p>
        <Link href="/resident/profile" className="mt-6 inline-block">
    ...
    return (
      <div className="space-y-5">
        <div>
          ...
          <h1 className="mt-1 text-3xl font-bold">Community shop</h1>
          <p className="mt-1 text-muted-foreground">
            Buy and sell with every resident in the community.
          </p>
        </div>
        <Card>
          <CardContent className="space-y-3 p-4">
            <div className="grid gap-3 sm:grid-cols-[1fr_160px_auto]">
              ...
              />
              <Button onClick={() => void createListing()}>List item</Button>
            </div>
            <input
              ref={listingFileRef}
              type="file"
            ...
            >
              ...
              {listingImage ? "Change item image" : "Add item image"}
            </Button>
            {listingImage && (
              <img
                src={listingImage}
                alt="New marketplace item preview"
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {listings.map((item) => (
            <Card key={item.id} className="overflow-hidden">
              <CardContent className="p-0">
                {Array.isArray(
                  (item as Listing & { imageUrls?: unknown }).imageUrls,
                ) &&
                  (
                    (item as Listing & { imageUrls?: unknown })
                      .imageUrls as string[]
                  )[0] && (
                    <img
                      src={
                ...
                <div className="p-5">
                  ...
                  <p className="mt-1 text-sm text-muted-foreground">
                    {item.seller?.name ?? "Resident seller"}
                  </p>
  ...
  if (view === "messages") {
    ...
    return (
      <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
              ...
              {conversations.map((conversation) => (
                ...
                >
                  <p className="font-medium">
                    {conversation.members?.[0]?.user.name ?? "Conversation"}
                  </p>
        ...
        <Card>
          <CardContent className="flex min-h-80 flex-col justify-end p-5">
            <div className="flex gap-2">
              <Input
                value={message}
  ...
  const suggestions = friends.filter(
    (friend) => friend.status === "SUGGESTION" || friend.status === "SENT",
  );
  const personCard = (
    person: FriendCard,
      ...
      <CardContent className="p-0">
        <div className="flex h-36 items-center justify-center bg-muted/40">
          {person.image ? (
            <img
              src={person.image}
              alt={`${person.name ?? "Resident"} profile`}
        ...
        <div className="p-3">
          <p className="truncate font-semibold">{person.name ?? "Resident"}</p>
          <p className="mt-1 text-xs text-muted-foreground">{detail}</p>
          <div className="mt-3 flex gap-2">{action}</div>
        </div>
      </CardContent>
    </Card>
  );
  return (
    <div className="space-y-7">
      <div>
        ...
        <h1 className="mt-1 text-3xl font-bold">Friends hub</h1>
        <p className="mt-1 text-muted-foreground">
          Connect with residents and discover mutual friends.
        </p>
      </div>
      {incoming.length > 0 && (
        <section>
          <div className="mb-3 flex items-center justify-between">
            ...
            <Badge>{incoming.length} pending</Badge>
          </div>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {incoming.map((person) =>
              personCard(
                ...
                <>
                  <Button
                    ...
                    className="flex-1"
                    onClick={() =>
                      void updateFriendRequest(person.requestId!, "ACCEPTED")
                    }
                  >
                    Confirm
                  </Button>
                  <Button
                    ...
                    className="flex-1"
                    onClick={() =>
                      void updateFriendRequest(person.requestId!, "DECLINED")
                    }
                  >
                    Delete
                  </Button>
      ...
      <section>
            ...
            personCard(
              ...
              person.status === "SENT" ? (
                <Button
                  ...
                  variant="outline"
                  className="w-full"
                  onClick={() => void removeFriendRequest(person.requestId!, person.id)}
                >
        ...
        {!loading && suggestions.length === 0 && (
          <p className="py-8 text-center text-sm text-muted-foreground">
            All available residents are already connected or have a pending
            request.
          </p>
        )}
      </section>
      <section>
        ...
        {!loading && accepted.length === 0 && (
          <p className="py-8 text-center text-sm text-muted-foreground">
            Accept requests to build your friend list.
          </p>
        )}
      </section>
    </div>
